...
class Listemployee_model extends CI_Model{
	function find($key=""){
		$this->db->select("*");
		if ( empty($key) == FALSE )
	...
	function _get_view_query(){
		...
		$__column_search 	= array('GroupKendaraanId', 'emp_nip', 'emp_name', 'JenisKendaraan', 'NoKendaraan');
		$__column_order     = array('GroupKendaraanId', 'emp_nip', 'emp_name', 'JenisKendaraan', 'NoKendaraan');
